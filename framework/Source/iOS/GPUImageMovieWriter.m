...
@interface GPUImageMovieWriter ()
{
    ...
    CMTime startTimeV, startTimeA, previousFrameTime;
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    startTimeV = kCMTimeInvalid;
    startTimeA = kCMTimeInvalid;
...
- (void)initializeMovieWithOutputSettings:(NSMutableDictionary *)outputSettings;
{
    _status = GPUImageMovieWriterStatusReady;
...
- (void)startRecording;
{
    NSAssert(self.status == GPUImageMovieWriterStatusReady, @"Starting recording twice");
    
    alreadyFinishedRecording = NO;
    dispatch_sync(movieWritingQueue, ^{
        _status = GPUImageMovieWriterStatusRecording;
        _timeOffsetA = CMTimeMake(0, 0);
        _timeOffsetV = CMTimeMake(0, 0);
        startTimeV = kCMTimeInvalid;
        startTimeA = kCMTimeInvalid;
    });
...
- (void)pauseRecording
{
    NSLog(@"Info: pause 1");
    dispatch_sync(movieWritingQueue, ^{
        if (_status != GPUImageMovieWriterStatusRecording)
        {
            return ;
        }
        
        _status = GPUImageMovieWriterStatusPaused;
        _discontA = YES;
        _discontV = YES;
        NSLog(@"Info:  pause 2");
    });
...
- (void)resumeRecording
{
    NSLog(@"Info: resume  1");
    dispatch_sync(movieWritingQueue, ^{
        if (_status != GPUImageMovieWriterStatusPaused)
        {
            return ;
        }
...
- (void)cancelRecording;
{
    ...
    dispatch_sync(movieWritingQueue, ^{
        ...
        _status = GPUImageMovieWriterStatusCanceled;
...
- (void)finishRecordingWithCompletionHandler:(void (^)(void))handler;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        _status = GPUImageMovieWriterStatusFinished;
        dispatch_sync(movieWritingQueue, ^{
            [assetWriterVideoInput markAsFinished];
            [assetWriterAudioInput markAsFinished];
...
- (void)processAudioBuffer:(CMSampleBufferRef)newAudioBuffer;
{
    __block CMSampleBufferRef audioBuffer = newAudioBuffer;
    ...
    dispatch_sync(movieWritingQueue, ^{
        ...
        if (_hasAudioTrack)
        {
            ...
            if (CMTIME_IS_INVALID(startTimeA))
            {
                NSLog(@"Info: start time from audio");
                if ((audioInputReadyCallback == NULL) && (assetWriter.status != AVAssetWriterStatusWriting))
                {
                    [assetWriter startWriting];
                }
                [assetWriter startSessionAtSourceTime:currentSampleTime];
                startTimeA = currentSampleTime;
            }
            ...
            if (_discontA)
            {
                ...
                NSLog(@"Info: a2 %f", CMTimeGetSeconds(pts));
                CMTime last = _lastAudio;
                if (last.flags & kCMTimeFlags_Valid)
                {
                    if (_timeOffsetA.flags & kCMTimeFlags_Valid)
                    {
                        pts = CMTimeSubtract(pts, _timeOffsetA);
                    }
                    CMTime offset = CMTimeSubtract(pts, last);
                    NSLog(@"Setting offset from audio");
                    NSLog(@"Adding %f to %f (pts %f)", ((double)offset.value)/offset.timescale, ((double)_timeOffsetA.value)/_timeOffsetA.timescale, ((double)pts.value/pts.timescale));
                    
                    // this stops us having to set a scale for _timeOffset before we see the first video time
                    if (_timeOffsetA.value == 0)
                    {
                        _timeOffsetA = offset;
                    }
                    else
                    {
                        _timeOffsetA = CMTimeAdd(_timeOffsetA, offset);
                    }
                }
                _lastAudio.flags = 0;
//                _lastVideo.flags = 0;
            ...
            if (_timeOffsetA.value > 0)
            {
                ...
                audioBuffer = [self adjustTime:audioBuffer by:_timeOffsetA];
                NSLog(@"Info: a4");
            }
...
//            dispatch_async(movieWritingQueue, ^{
                NSLog(@"Info: a6 %f    %f", CMTimeGetSeconds(pts), CMTimeGetSeconds(_timeOffsetA));
                BOOL result = [assetWriterAudioInput appendSampleBuffer:audioBuffer];
            if (!result)
            {
                NSLog(@"problem appending audio buffer");
            }
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    dispatch_sync(movieWritingQueue, ^{
        ...
        if (CMTIME_IS_INVALID(startTimeV))
        {
            NSLog(@"Info: start time from video");
            //        dispatch_sync(movieWritingQueue, ^{
    ...
    else
    {
        ...
        if (_discontV) {
            ...
            if (YES){//!self.hasAudioTrack) {
                ...
                {
                    NSLog(@"Info: va2");
                    if (_timeOffsetV.flags & kCMTimeFlags_Valid)
                    {
                        pts = CMTimeSubtract(pts, _timeOffsetV);
                    }
                    CMTime offset = CMTimeSubtract(pts, last);
                    NSLog(@"Setting offset from video");
                    NSLog(@"Adding %f to %f (pts %f)", ((double)offset.value)/offset.timescale, ((double)_timeOffsetV.value)/_timeOffsetV.timescale, ((double)pts.value/pts.timescale));
                    
                    // this stops us having to set a scale for _timeOffset before we see the first video time
                    if (_timeOffsetV.value == 0)
                    {
                        _timeOffsetV = offset;
                    }
                    else
                    {
                        _timeOffsetV = CMTimeAdd(_timeOffsetV, offset);
                    }
                }
            }
            _lastVideo.flags = 0;
        ...
        {
            NSLog(@"Info: v5");
            _lastVideo = CMTimeSubtract(frameTime, _timeOffsetV);
        }
        else
        {
        ...
        CMTime time = CMTimeSubtract(_lastVideo, startTimeV);
        _recordedSeconds = CMTimeGetSeconds(time);
        NSLog(@"videoFrame: ft %f - sv %f = %f\t - rt %f = to %f === %f    %i", CMTimeGetSeconds(frameTime), CMTimeGetSeconds(startTimeV), CMTimeGetSeconds(CMTimeSubtract(frameTime, startTimeV)), _recordedSeconds, CMTimeGetSeconds(_timeOffsetV), CMTimeGetSeconds(CMTimeSubtract(frameTime, _lastVideo)), assetWriterVideoInput.readyForMoreMediaData);
        ...
        {
            NSLog(@"Problem appending pixel buffer at time: %f", CMTimeGetSeconds(presentationTime));
            _error = assetWriter.error;
            dispatch_async(dispatch_get_main_queue(), ^{
                [self cancelRecording];
                if (self.delegate && [self.delegate respondsToSelector:@selector(movieRecordingFailedWithError:)])
                {
                    [self.delegate movieRecordingFailedWithError:self.error];
                }
            });
        }
        else
...
- (CGFloat)recordedSeconds
{
    __block CGFloat result = 0.0;
    dispatch_sync(movieWritingQueue, ^(){
        result = _recordedSeconds;
    });
    return result;
}

- (GPUImageMovieWriterStatus)status
{
    __block GPUImageMovieWriterStatus result;
    dispatch_sync(movieWritingQueue, ^{
        result = _status;
    });
    return result;
...
- (void)pauseRecording
{
    ...
    dispatch_barrier_async(movieWritingQueue, ^{
        _status = GPUImageMovieWriterStatusPausing;
    });
}
...
- (void)resumeRecording
{
    ...
    dispatch_barrier_async(movieWritingQueue, ^{
        _status = GPUImageMovieWriterStatusResuming;
    });
}
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    if (!isRecording)
    {
        if (_status == GPUImageMovieWriterStatusPausing)
        {
    ...
    if (_status == GPUImageMovieWriterStatusResuming)
    {
        _timeOffset = CMTimeSubtract(frameTime, _lastVideo);
        _status = GPUImageMovieWriterStatusRecording;
    }
    ...
    if (CMTIME_IS_INVALID(startTime))
    {
        dispatch_sync(movieWritingQueue, ^{
            ...
            [assetWriter startSessionAtSourceTime:frameTime];
            startTime = frameTime;
            _lastVideo = startTime;
            _timeOffset = CMTimeSubtract(_lastVideo, startTime);
        });
    }
    ...
    dispatch_sync(movieWritingQueue, ^{
        _lastVideo = CMTimeSubtract(frameTime, _timeOffset);
        _recordedSeconds = CMTimeGetSeconds(CMTimeSubtract(_lastVideo, startTime));
        if (_lastVideo.value == 0)
        {
            NSLog(@"Info: something wrong");
            return ;
        }
        
        if(![assetWriterPixelBufferInput appendPixelBuffer:pixel_buffer withPresentationTime:_lastVideo])
        {
            NSLog(@"Problem appending pixel buffer at time: %lld, video time %lld \nError %@", frameTime.value, _lastVideo.value, assetWriter.error);
            _error = assetWriter.error;
            if ([self.delegate respondsToSelector:@selector(movieRecordingFailedWithError:)])
            {
                [self.delegate movieRecordingFailedWithError:_error];
