...
NSString *const kGPUImageSmartEyeSkinDetectSmoothCSEnhanceSaveFragmentShaderString = SHADER_STRING
(
 ...
 void main()
 {
     ...
     {
         count += 1;
         gaussianWeightTotal += 0.015;
         sum += centralColor * 0.015;
     }else{
         distanceFromCentralColor = min(distance(centralColor, sampleColor) * distanceNormalizationFactor, 1.0);
         gaussianWeight = 0.015 * (1.0 - distanceFromCentralColor);
         gaussianWeightTotal += gaussianWeight;
         sum += sampleColor * gaussianWeight;
     }
     //-0.5 -1
     singleStepOffset = vec2(textureCoordinate.x -0.5*step*mXStep, textureCoordinate.y -step*mYStep);
     sampleColor = texture2D(inputImageTexture, singleStepOffset);
     ...
     {
         count += 1;
         gaussianWeightTotal += 0.025;
         sum += centralColor * 0.025;
     }else{
         distanceFromCentralColor = min(distance(centralColor, sampleColor) * distanceNormalizationFactor, 1.0);
         gaussianWeight = 0.025 * (1.0 - distanceFromCentralColor);
         gaussianWeightTotal += gaussianWeight;
         sum += sampleColor * gaussianWeight;
     }
     //0.5  -1
     ...
     //1 -0.5
     singleStepOffset = vec2(textureCoordinate.x +step*mXStep, textureCoordinate.y - 0.5*step*mYStep);
     sampleColor = texture2D(inputImageTexture, singleStepOffset);
     ...
     //-0.5 -0.5
     singleStepOffset = vec2(textureCoordinate.x -0.5*step*mXStep, textureCoordinate.y -0.5*step*mYStep);
     sampleColor = texture2D(inputImageTexture, singleStepOffset);
     
     if((sampleColor.x < test)&&(centralColor.x < test))
     {
         count += 1;
         gaussianWeightTotal += 0.04;
         sum += centralColor * 0.04;
     }else{
         distanceFromCentralColor = min(distance(centralColor, sampleColor) * distanceNormalizationFactor, 1.0);
         gaussianWeight = 0.04 * (1.0 - distanceFromCentralColor);
         gaussianWeightTotal += gaussianWeight;
         sum += sampleColor * gaussianWeight;
     }
     //0 -0.5
...
@implementation GPUImageSmartEyeSkinDetectSmoothCSEnhanceSave
...
- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:kGPUImageSmartEyeSkinDetectSmoothCSEnhanceSaveFragmentShaderString]))
    {
        return nil;
    }
