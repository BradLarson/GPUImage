...
//    if (captureAsYUV && [GPUImageContext deviceSupportsRedTextures])
    ...
    if (captureAsYUV)
    {
        [self setYUVConversionProgram:nil];
    }
...
- (void)setYUVConversionProgram:(NSString *)conversion
{
    if (conversion == nil)
    {
        conversion = isFullYUVRange ? kGPUImageYUVFullRangeConversionForLAFragmentShaderString : kGPUImageYUVVideoRangeConversionForLAFragmentShaderString;
    }
    ...
    if (captureAsYUV)
    {
        runSynchronouslyOnVideoProcessingQueue(^{
          ...
          [GPUImageContext useImageProcessingContext];
          yuvConversionProgram = [[GPUImageContext sharedImageProcessingContext]  programForVertexShaderString:kGPUImageVertexShaderString fragmentShaderString:conversion];
...
{
    if (captureAsYUV && block)
    {
        runSynchronouslyOnVideoProcessingQueue(^{
            [GPUImageContext useImageProcessingContext];
            block(yuvConversionProgram);
        });
    }
}
